...
public interface Validator<T, C> {
     ...
     */
    default ServiceResult<T> doValidate(ValidatorContext<C> context, ValidatorChain chain) throws Exception {
        ...
        //否则往责任链中下一个校验器进行处理
        return chain.doValidate(context);
    }

    /**
     * 不需要责任链的时候，则可以直接调用此方法的实现即可
     */
    default ServiceResult<T> doValidate(ValidatorContext<C> context) throws Exception {
